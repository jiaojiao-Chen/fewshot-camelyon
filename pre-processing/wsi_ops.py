...
class PatchExtractor(object):
    @staticmethod
    def extract_positive_patches_from_tumor_region(wsi_image, wsi_mask, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            area = (b_y_end - b_y_start) * (b_x_end - b_x_start)
            sum += area
        ...
        for bounding_box in bounding_boxes:
            ...
            b_y_end = int(bounding_box[1]) + int(bounding_box[3])
            area = (b_y_end - b_y_start) * (b_x_end - b_x_start)
            size = int(2000 * area / sum)
            X = np.random.random_integers(b_x_start, high=b_x_end, size = size)
            Y = np.random.random_integers(b_y_start, high=b_y_end, size = size)

            for x, y in zip(X, Y):
                mask = wsi_mask.read_region((x * mag_factor, y * mag_factor), 0, (utils.PATCH_SIZE, utils.PATCH_SIZE))
                mask_gt = np.array(mask)
                mask_gt = cv2.cvtColor(mask_gt, cv2.COLOR_BGR2GRAY)
                white_pixel_cnt_gt = cv2.countNonZero(mask_gt)
                if white_pixel_cnt_gt > (utils.PATCH_SIZE*utils.PATCH_SIZE*0.50):
                    patch = wsi_image.read_region((x * mag_factor, y * mag_factor), 0,
                                                  (utils.PATCH_SIZE, utils.PATCH_SIZE))
                    patch.save(patch_save_dir + patch_prefix + str(patch_index)+'.png', 'PNG')
                    patch_index += 1
                    patch.close()
    ...
    @staticmethod
    def extract_negative_patches_from_tumor_wsi(wsi_image, wsi_mask, image_open, level_used,
                                                bounding_boxes, patch_save_dir, patch_prefix,
                                                patch_index):
        """
        ...
        for bounding_box in bounding_boxes:
            b_x_start = int(bounding_box[0])
            b_y_start = int(bounding_box[1])
            b_x_end = int(bounding_box[0]) + int(bounding_box[2])-3
            b_y_end = int(bounding_box[1]) + int(bounding_box[3])-3
            area = (b_y_end - b_y_start) * (b_x_end - b_x_start)
            size = int(1000 * area / sum)
            X = np.random.random_integers(b_x_start, high=b_x_end, size=size)
            Y = np.random.random_integers(b_y_start, high=b_y_end, size=size)
            ...
            for x, y in zip(X, Y):
                ...
                if white_pixel_cnt_gt == 0:
                    ...
                    if white_pixel_cnt > ((utils.PATCH_SIZE * utils.PATCH_SIZE) * 0.20):
                        patch.save(patch_save_dir + patch_prefix + str(patch_index)+'.png', 'PNG')
                        patch_index += 1
                        patch.close()
    ...
    def extract_patches_from_heatmap_false_region_tumor(wsi_image, wsi_mask, tumor_gt_mask, image_open,
                                                        ...
                                                        patch_index_pos,patch_index_neg):
        """
        ...
        for bounding_box in bounding_boxes:
            b_x_start = int(bounding_box[0])
            b_y_start = int(bounding_box[1])
            b_x_end = int(bounding_box[0]) + int(bounding_box[2])-2
            b_y_end = int(bounding_box[1]) + int(bounding_box[3])-2
            col_cords = np.arange(b_x_start, b_x_end, 2)
            row_cords = np.arange(b_y_start, b_y_end, 2)
                ...
                for col in col_cords:
                    if int(image_open[row, col]) is not utils.PIXEL_BLACK:  # consider pixels from ROI only
                        ...
                        if heatmap_prob[row, col] >= utils.TUMOR_PROB_THRESHOLD\
                                or heatmap_prob[row+1, col] >= utils.TUMOR_PROB_THRESHOLD\
                                or heatmap_prob[row, col+1] >= utils.TUMOR_PROB_THRESHOLD\
                                or heatmap_prob[row+1, col+1] >= utils.TUMOR_PROB_THRESHOLD:
                            if int(tumor_gt_mask[row, col]) == utils.PIXEL_BLACK:
                                ...
                                if white_pixel_cnt_gt == 0:
                                    patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                                  (utils.PATCH_SIZE, utils.PATCH_SIZE))
                                    patch.save(patch_save_dir_neg + patch_prefix_neg + str(patch_index_neg)+'.png', 'PNG')
                                    patch_index_neg += 1
                                    patch.close()

                                mask.close()
                        # extract patch corresponds to false negatives
                        elif (int(tumor_gt_mask[row, col]) is not utils.PIXEL_BLACK or \
                                int(tumor_gt_mask[row+1, col]) is not utils.PIXEL_BLACK or \
                                int(tumor_gt_mask[row, col+1]) is not utils.PIXEL_BLACK or \
                                int(tumor_gt_mask[row+1, col+1]) is not utils.PIXEL_BLACK):
                            # mask_gt does not contain tumor area
                            mask = wsi_mask.read_region((col * mag_factor, row * mag_factor), 0,
                                                        (utils.PATCH_SIZE, utils.PATCH_SIZE))
                            mask_gt = cv2.cvtColor(np.array(mask), cv2.COLOR_BGR2GRAY)
                            ...
                            if white_pixel_cnt_gt >= ((utils.PATCH_SIZE * utils.PATCH_SIZE) * 0.85):
                                patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                              (utils.PATCH_SIZE, utils.PATCH_SIZE))
                                patch.save(patch_save_dir_pos + patch_prefix_pos + str(patch_index_pos)+'.png', 'PNG')
                                patch_index_pos += 1
                                patch.close()
                ...
                for col in col_cords:
                    if int(image_open[row, col]) is not utils.PIXEL_BLACK:  # consider pixels from ROI only
                        ...
                        if heatmap_prob[row, col] >= utils.TUMOR_PROB_THRESHOLD\
                            ...
                            patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                          (utils.PATCH_SIZE, utils.PATCH_SIZE))
                            patch.save(patch_save_dir_neg + patch_prefix_neg + str(patch_index)+'.png', 'PNG')
                            patch_index += 1
                            patch.close()
